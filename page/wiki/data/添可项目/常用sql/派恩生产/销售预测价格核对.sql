SET @p_node_id = 145;     -- 你要查询的节点ID
SET @p_week = '2026W24';  -- 你要查询的周次
# select * from node
SELECT
    COALESCE(a.sap_id, b.sapId) AS sap_id,
    IFNULL(a.拆分合计价格ai, 0) AS 拆分合计价格ai,
    IFNULL(a.拆分合计价格人工, 0) AS 拆分合计价格人工,
    IFNULL(b.预测合计价格ai, 0) AS 预测合计价格ai,
    IFNULL(b.预测合计价格人工, 0) AS 预测合计价格人工,
    CASE
        WHEN IFNULL(a.拆分合计价格ai, 0) != IFNULL(b.预测合计价格ai, 0)
             OR IFNULL(a.拆分合计价格人工, 0) != IFNULL(b.预测合计价格人工, 0)
        THEN '有差异' ELSE '一致' END AS 差异状态
FROM
(
    -- 1. 拆分表：按 sap_id 汇总【价格】
    SELECT
        sap_id,
        SUM(price_ai) AS 拆分合计价格ai,
        SUM(price_made) AS 拆分合计价格人工
...
(
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE a.step_index = 1     -- 价格固定 step_index=1
...
-- HAVING 拆分合计价格ai != 预测合计价格ai OR 拆分合计价格人工 != 预测合计价格人工
ORDER BY sap_id;