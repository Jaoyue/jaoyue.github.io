...
-- 添可
select LEFT(create_time,10) 时间,source_code 售后单号,sap_id 物料号,content 故障类型,solution 智能体结果,FROM_UNIXTIME(start_time/1000,'%Y-%m-%d %H:%i:%s') 开始时间,FROM_UNIXTIME(end_time/1000,'%Y-%m-%d %H:%i:%s') 结束时间,(end_time-start_time)/1000 执行时间 from pn_obm_mro_process where create_time >= '2025-07-01 00:00:00' and create_time < '2025-07-08 00:00:00' and source_sys = 'CSS' and type = 'MRO';
select LEFT(create_time,10) 时间,source_code 售后单号,sap_id 物料号,content_type 故障类型,content 故障描述,solution 智能体结果,FROM_UNIXTIME(start_time/1000,'%Y-%m-%d %H:%i:%s') 开始时间,FROM_UNIXTIME(end_time/1000,'%Y-%m-%d %H:%i:%s') 结束时间,(end_time-start_time)/1000 执行时间 from pn_obm_mro_process where create_time >= '2025-07-01 00:00:00' and create_time < '2025-07-08 00:00:00' and source_sys = 'CSS' and type = 'OBM';
 -- 科沃斯
 select LEFT(create_time,10) 时间,source_code 售后单号,sap_id 物料号,content_type 故障类型,content 故障描述,solution 智能体结果,FROM_UNIXTIME(start_time/1000,'%Y-%m-%d %H:%i:%s') 开始时间,FROM_UNIXTIME(end_time/1000,'%Y-%m-%d %H:%i:%s') 结束时间,(end_time-start_time)/1000 执行时间 from pn_obm_mro_process where create_time >= '2025-06-16 15:55:51' and create_time < '2025-07-08 00:00:00' and source_sys = 'doscs' and type = 'OBM' and sn is not null;
